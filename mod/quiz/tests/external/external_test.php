...
class external_test extends externallib_advanced_testcase {
    ...
    /**
     * Test that a sequential navigation quiz is not allowing to see questions in advance but a teacher can
     */
    public function test_sequential_navigation_view_attempt() {
        ...
        // Now with teacher role.
        $this->setUser($this->teacher);
        $attemptobj = $this->create_quiz_attempt_object($quiz, $this->teacher->id);
        // Check out of sequence access to view the same attempt as a teacher.
        $this->assertNotEmpty(mod_quiz_external::view_attempt($attemptobj->get_attemptid(), 1, []));
        $this->assertNotEmpty(mod_quiz_external::view_attempt($attemptobj->get_attemptid(), 4, []));
    }

    /**
     * Test that a sequential navigation quiz is not allowing to see questions in advance for a student
     * but would do for a teacher
     */
    public function test_sequential_navigation_attempt_summary() {
        ...
        // Now start an attempt as a teacher.
        $attemptobj = $this->create_quiz_attempt_object($quiz, $this->teacher->id);
        $this->setUser($this->teacher);
        // Check that we can see all questions as a teacher.
        $result = mod_quiz_external::get_attempt_summary($attemptobj->get_attemptid());
        $this->assertCount(5, $result['questions']);
        foreach ($result['questions'] as $index => $question) {
            $pageindex = $index + 1;
            $this->assertStringContainsString("Question ({$pageindex})", $question['html']);
        }
    }
    ...
    public function test_sequential_navigation_get_attempt_data() {
        ...
        // But as a teacher you can see everything.
        $this->setUser($this->teacher);
        $attemptobj = $this->create_quiz_attempt_object($quiz, $this->teacher->id);
        // Check that we can see all questions as a teacher.
        $result = mod_quiz_external::get_attempt_summary($attemptobj->get_attemptid());
    ...
    private function create_quiz_attempt_object(quiz $quizobj, ?int $userid = null, ?bool $ispreview = false): quiz_attempt {
        ...
        $quba = \question_engine::make_questions_usage_by_activity('mod_quiz', $quizobj->get_context());
        $quba->set_preferred_behaviour($quizobj->get_quiz()->preferredbehaviour);
        $attempt = quiz_create_attempt($quizobj, 1, false, $timenow, $ispreview, $userid ?? $this->student->id);
        quiz_start_new_attempt($quizobj, $quba, $attempt, 1, $timenow);
        quiz_attempt_save_started($quizobj, $quba, $attempt);
        $attemptobj = quiz_attempt::create($attempt->id);
        return $attemptobj;
