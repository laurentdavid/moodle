...
class sectionactions extends baseactions {
     ...
     *
     * @param section_info $section the section to move
     * @param int $destination the position to move the section to
     * @return bool whether section was moved
     */
    public function move_at(section_info $section, int $destination): bool {
        global $DB;
        if (!course_get_format($this->course->id)->uses_sections()) {
            return false;
        }
        if ($section->sectionnum == 0 || $destination == 0) {
            return false;
        }
        if ($section->course != $this->course->id) {
            return false;
        }
        if ($section->sectionnum == $destination) {
            return false;
        }
        $modinfo = get_fast_modinfo($this->course->id);
        $allsections = $modinfo->get_section_info_all();
        if (count($allsections) <= $destination) {
            return false;
        }
