 ...
 */
class query_set_nav_anchor_to_referer extends \core\router\schema\parameters\query_parameter implements
    ...
    referenced_object
{
    ...
    public function __construct(
        string $name = 'setnavanchoreferer',
        ...$extra,
    ) {
        $extra['name'] = $name;
    ...
    public function add_attributes_for_parameter_value(
        ...
        return $request->withAttribute($this->name, $value);
    }
    ...
    protected function update_request_params(
        ...
        $request = parent::update_request_params($request, $params);
        if (isset($params[$this->name])) {
            $request = $this->add_attributes_for_parameter_value($request,  $params[$this->name]);
        }
        return $request;
    }
}
