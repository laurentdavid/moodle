...
class navigation_anchor_manager {
    ...
    /**
     * Query parameter name to set an anchor from the referer URL.
     */
    public const NAV_ANCHOR_SET_TO_REFERER_PARAMETER = 'setnavanchorreferer';
     ...
     */
    public static function set(string $key, url $url): void {
        global $SESSION;
        if (!$url->is_local_url()) {
            return;
        }
     ...
     */
    public static function set_referer_from_request(ServerRequestInterface $request): void {
        $queryparams = $request->getQueryParams();
        $anchorkey = $queryparams[self::NAV_ANCHOR_SET_TO_REFERER_PARAMETER] ?? null;
        if ($anchorkey) {
    ...
    /**
     * Add the setnavanchorreferer parameter to a URL.
     *
     * @param url $url the URL to modify
     * @param string $key the anchor key to set
     * @return url the modified URL
     */
    public static function add_set_anchor_to_referer(url $url, string $key) {
        $url->param(self::NAV_ANCHOR_SET_TO_REFERER_PARAMETER, $key);
        return $url;
    }
}
