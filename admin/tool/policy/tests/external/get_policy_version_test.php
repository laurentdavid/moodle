 ...
 */
class get_policy_version_test extends externallib_advanced_testcase {
     ...
     * @covers \tool_policy\external\get_policy_version::execute
     */
    public function test_get_policy_version() {
        $this->setUser($this->adult);

        // View current policy version.
        $result = get_policy_version::execute($this->policy2->get('id'));
        $result = \external_api::clean_returnvalue(get_policy_version::execute_returns(), $result);
        ...
        $result = get_policy_version::execute($this->policy3->get('id'));
        $result = \external_api::clean_returnvalue(get_policy_version::execute_returns(), $result);
        ...
        $result = get_policy_version::execute(999);
        $result = \external_api::clean_returnvalue(get_policy_version::execute_returns(), $result);
        $this->assertCount(0, $result['result']);
        $this->assertCount(1, $result['warnings']);
        ...
        $result = get_policy_version::execute($this->policy1->get('id'), $this->child->id);
        $result = \external_api::clean_returnvalue(get_policy_version::execute_returns(), $result);
        $this->assertCount(0, $result['result']);
        $this->assertCount(1, $result['warnings']);
        $this->assertEquals(array_pop($result['warnings'])['warningcode'], 'errorusercantviewpolicyversion');
