...
M.availability_date.form.findAllDateSiblings = function(tree, nodeUIDToFind) {
    ...
    var siblingsFinderRecursive = function(itemsTree) {
        ...
        var index;
        var childDates;
        for (index = 0; index < itemsTree.length; index++) {
            itemValue = itemsTree.at(index);
            if (itemValue.type === undefined) {
                childDates = siblingsFinderRecursive(itemValue.c);
                if (childDates) {
                    return childDates;
                }
            }
...
M.availability_date.form.checkConditionDate = function(currentNode) {
    ...
    if (M.core_availability.form.rootList.getValue().op === '&') {
        ...
        var dateSiblings = M.availability_date.form.findAllDateSiblings(jsValue, currentNodeUID, 0);
        if (dateSiblings) {
