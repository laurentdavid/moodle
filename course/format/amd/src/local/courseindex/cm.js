...
export default class Component extends DndCmItem {
    ...
    _getCmIndexURL() {
        ...
        if (!cm.url) {
            ...
            if (!elementInPage) {
                // We are not in the course Index, so we should jump to the course index with the right anchor.
                if (!currentURL.pathname.includes(course.baseurl)) {
                    return new URL(`${course.baseurl}&section=${section.number}#${cm.anchor}`);
                } else {
