...
function xmldb_main_upgrade($oldversion) {
    ...
    if ($oldversion < 2025032100.01) {
        ...
        if (!file_exists($CFG->dirroot . "/mod/survey/version.php")) {
            ...
            $sqlparams['surveypluginname'] = '%' . $DB->sql_like_escape('survey') . '%';
            $sqllikes[] = $DB->sql_like('indicators', ':surveypluginname');
        }
        if (!file_exists($CFG->dirroot . "/mod/chat/version.php")) {
            ...
            $sqlparams['chatpluginname'] = '%' . $DB->sql_like_escape('chat') . '%';
            $sqllikes[] = $DB->sql_like('indicators', ':chatpluginname');
        }

        foreach ($indicatorstoremove as $module => $indicators) {
            if (!file_exists($CFG->dirroot . "/mod/{$module}/version.php")) {
                $models = $DB->get_recordset_select('analytics_models', join(' OR ', $sqllikes), $sqlparams);
